...
int	load_assets(t_game *game)
{
	if (!load_image(game->mlx_ptr, &game->img.floor, game, "assets/tile.xpm")
		|| !load_image(game->mlx_ptr, &game->img.player, game,
			"assets/player.xpm")
		|| !load_image(game->mlx_ptr, &game->img.collectible, game,
			"assets/collectible.xpm")
		|| !load_image(game->mlx_ptr, &game->img.wall, game,
			"assets/wall.xpm")
		|| !load_image(game->mlx_ptr, &game->img.exit, game,
			"assets/exit.xpm"))
	{
		return (0);
	}
